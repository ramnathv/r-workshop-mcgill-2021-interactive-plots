...
Given cuisine, nb_ingredients -> Data Frame with Top Ingredients
Given cuisine -> Word Cloud of Ingredients
Given cuisine, nb_ingredients -> Interactive Plot of Top Ingredients

```{r}
#' Summarize top N ingredients in a given recipe
#' 
#' @param recipes A data frame of recipes with three columns
#' @param selected_cuisine The cuisine selected to be summarized
#' @param nb_ingredients The max number of top ingredients to summarize
summarize_cuisine <- function(recipes, 
                              selected_cuisine = 'greek', 
                              nb_ingredients = 10) {
  recipes %>% 
    filter(cuisine == selected_cuisine) %>% 
    # group_by(ingredient) %>% 
    # summarize(nb_recipes = n()) %>% 
    count(ingredient, name = 'nb_recipes') %>% 
    slice_max(nb_recipes, n = nb_ingredients)
}

recipes %>% 
  summarize_cuisine('indian')
```
...
plot_top_ingredients_tfidf <- function(df){
  p <- df %>% 
    mutate(ingredient = forcats::fct_reorder(ingredient, tf_idf)) %>% 
    ggplot(aes(x = tf_idf, y = ingredient)) +
    geom_col()

  plotly::ggplotly(p)
}

plot_top_ingredients_tfidf(df)
```
...
server <- function(input, output, session) {
  ...
  rval_top_ingredients <- reactive({
    recipes_enriched %>% 
      ...
      slice_max(tf_idf, n = input$nb_ingredients)
  })
  ...
  output$plot_top_ingredients <- renderPlotly({
    rval_top_ingredients() %>%
      plot_top_ingredients_tfidf()
  })
...
```{r}
plot_word_cloud <- function(selected_cuisine = 'greek', nb_ingredients = 50){
  recipes_enriched %>% 
    filter(cuisine == selected_cuisine) %>% 
    rename(word = ingredient, freq = tf_idf) %>%
    slice_max(freq, n = nb_ingredients) %>% 
    select(word, freq) %>% 
  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel('WordCloud', wordcloud2::wordcloud2Output('wordcloud_top_ingredients')),
        tabPanel('Plot', plotly::plotlyOutput('plot_top_ingredients')),
        tabPanel('Table', DT::DTOutput('dt_top_ingredients'))
      )
    )
...
server <- function(input, output, session){
  ...
  output$wordcloud_top_ingredients <- renderWordcloud2({
    plot_word_cloud(input$cuisine, input$nb_ingredients)
  })
